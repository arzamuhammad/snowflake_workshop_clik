...
-- 2) COPY INTO tables (COPY FILES mempertahankan struktur folder sumber,
--    jadi file berada di @RAW_DATA_STAGE/01_data_generation/data/)
COPY INTO DIM_REGION FROM @RAW_DATA_STAGE/01_data_generation/data/dim_region.csv FILE_FORMAT=(FORMAT_NAME=CSV_FF) FORCE=TRUE;
COPY INTO DIM_PRODUCT FROM @RAW_DATA_STAGE/01_data_generation/data/dim_product.csv FILE_FORMAT=(FORMAT_NAME=CSV_FF) FORCE=TRUE;
COPY INTO DIM_LENDER FROM @RAW_DATA_STAGE/01_data_generation/data/dim_lender.csv FILE_FORMAT=(FORMAT_NAME=CSV_FF) FORCE=TRUE;
COPY INTO LOAN_APPLICATIONS FROM @RAW_DATA_STAGE/01_data_generation/data/loan_applications.csv FILE_FORMAT=(FORMAT_NAME=CSV_FF) FORCE=TRUE;
